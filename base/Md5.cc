...
uint32_t Md5Encode::fillData(const char *inDataPtr, int dataByteLen,
                             char **outDataPtr) {
  ...
  if (0 < bitNeedFill) {
    *outDataPtr = new char[allBit / BIT_OF_BYTE + SRC_DATA_LEN / BIT_OF_BYTE];
    memset(*outDataPtr, 0, allBit / BIT_OF_BYTE + SRC_DATA_LEN / BIT_OF_BYTE);
    // copy data
