...
const char *strerror_tl(int savedErrno) { return strerror(savedErrno); }
...
void Logger::formatTime() {
  ...
  if (now != lastSecond_) {
    lastSecond_ = now;
    strncpy(lastTimeString_, date_.toFormattedString(false).c_str(),
            sizeof(lastTimeString_) - 1);
  }
  logStream_ << T(lastTimeString_, 17);
  char tmp[32];
  snprintf(tmp, sizeof(tmp), ".%06llu UTC ",
...
LogStream &Logger::stream() { return logStream_; }
