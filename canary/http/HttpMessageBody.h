...
class HttpMessageBody {
 public:
  ...
  BodyType bodyType() { return type_; }
...
class HttpMessageStringBody : public HttpMessageBody {
 public:
  HttpMessageStringBody() { type_ = BodyType::kString; }
  ...
  const char *data() const override { return body_.data(); }
  char *data() override { return const_cast<char *>(body_.data()); }
  size_t length() const override { return body_.length(); }
  const std::string &getString() const override { return body_; }
  std::string &getString() override { return body_; }
  void append(const char *buf, size_t len) override { body_.append(buf, len); }
