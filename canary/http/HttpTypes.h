...
inline string_view to_string_view(canary::ReqResult result) {
  switch (result) {
    case ReqResult::Ok:
      return "OK";
...
inline std::string to_string(canary::ReqResult result) {
  return to_string_view(result).data();
}

inline std::ostream &operator<<(std::ostream &out, canary::ReqResult result) {
  return out << to_string_view(result);
}

inline canary::LogStream &operator<<(canary::LogStream &out,
                                     canary::ReqResult result) {
  return out << to_string_view(result);
}
