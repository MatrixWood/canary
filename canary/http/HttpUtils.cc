...
ContentType getContentType(const std::string &fileName) {
  ...
  if (pos != std::string::npos) {
    ...
    transform(extName.begin(), extName.end(), extName.begin(),
  ...
  switch (extName.length()) {
    ...
    case 2: {
      if (extName == "js") return CT_APPLICATION_X_JAVASCRIPT;
      return CT_APPLICATION_OCTET_STREAM;
    }
    case 3: {
      switch (extName[0]) {
        case 'b':
          if (extName == "bmp") return CT_IMAGE_BMP;
          break;
        case 'c':
          if (extName == "css") return CT_TEXT_CSS;
          break;
        case 'e':
          if (extName == "eot") return CT_APPLICATION_VND_MS_FONTOBJ;
          break;
        case 'g':
          if (extName == "gif") return CT_IMAGE_GIF;
          break;
        case 'i':
          if (extName == "ico") return CT_IMAGE_XICON;
          break;
        case 'j':
          if (extName == "jpg") return CT_IMAGE_JPG;
          break;
        case 'o':
          if (extName == "otf") return CT_APPLICATION_X_FONT_OPENTYPE;
          break;
        case 'p':
          if (extName == "png")
            return CT_IMAGE_PNG;
          ...
          break;
        case 's':
          if (extName == "svg") return CT_IMAGE_SVG_XML;
          break;
        case 't':
          if (extName == "txt")
    ...
    case 5: {
      if (extName == "woff2") return CT_APPLICATION_FONT_WOFF2;
      return CT_APPLICATION_OCTET_STREAM;
    }
    default:
      return CT_APPLICATION_OCTET_STREAM;
...
FileType parseFileType(const string_view &fileExtension) {
  ...
  if (iter == map_.end()) return FT_CUSTOM;
  return iter->second;
}
...
std::pair<ContentType, const string_view> fileNameToContentTypeAndMime(
  ...
  auto it = customMime.find(extName);
  if (it == customMime.end()) return {CT_NONE, ""};
